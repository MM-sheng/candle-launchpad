...
Decision: external audit is deferred. Launch is unaudited, for a small informed initial group. Contracts remain frozen at `v1.0.0-rc1`. An unaudited UI warning and completed provider ownership transfer to a Safe multisig are launch requirements. Small scale is an operational policy, not a protocol-wide cap.

Status legend: ✅ done · 🔧 engineering, can be done now · 🧑 needs a human / external party · ⏳ waiting

Preparation guide: [中文账号与部署准备](MAINNET-PREP.zh.md).
...
| 1.4 | Freeze the code: tag `v1.0.0-rc1`, no further contract changes under the current launch scope | ✅ tagged |
| 1.5 | **External audit** of `src/` (one reputable firm; budget 2–4 weeks). Hand them `README.md`, this file, `test/`, and the Slither report | Deferred by user decision |
| 1.6 | Fix audit findings, re-run the full suite + fuzz with `FOUNDRY_PROFILE=ci` (10k runs), tag `v1.0.0` | Deferred with audit |

Known accepted risks (document for auditors and in the UI):
- Auction creation is permissionless: anyone can list any token. The contract includes deferred-delivery and refund paths; these are not a guarantee against arbitrary malicious tokens or undiscovered bugs, but it cannot make a worthless token valuable.
...
| 2.2 | Create a mainnet VRF subscription at https://vrf.chain.link, fund with BNB (`payInNative=true`) — funding amount must be checked against the live subscription estimate before funding; no fixed total budget is approved | 🧑 |
| 2.3 | Deploy `ChainlinkVRFProvider` + `CandleAuctionHouse` (`script/Deploy.s.sol`), add the provider as a consumer | 🧑 |
| 2.4 | **Transfer provider ownership to a Safe multisig**; complete both transfer and acceptance. The base `VRFConsumerBaseV2Plus.setCoordinator` cannot be disabled; whoever owns the provider could point it at a fake coordinator and choose the cutoff. Ownership must not sit on a hot wallet | 🧑 |
...
| 3.3 | Crank wallet: dedicated key, gas budget checked before funding, balance alert | 🧑 |
...
| 4.8 | Global unaudited banner, no refund guarantee, explain 30-day failed-delivery waiting period | 🔧 |
| 4.7 | Terms / risk disclosure page; jurisdiction review for token sales | 🧑 |

## 5. Launch sequence

1. Frozen `v1.0.0-rc1` contract sources verified; tests/build pass; two-wallet browser acceptance completed; unaudited risk disclosure ready. No audit sign-off is claimed.
2. Deploy to mainnet from a fresh deployer key; verify on BscScan; record addresses in `docs/mainnet.md`.
3. Provider ownership → Safe multisig (accept ownership and verify `owner()`); VRF subscription funded; consumer added.
...
6. Invite a small informed initial group. Keep the first public auctions small (`minRaise` and supply caps are per-auction parameters).
